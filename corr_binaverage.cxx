...
*/
    bootstrapping(corr, nbin, nsample, av1, er1);
      
      datafl<< setw(20) << setprecision(14) << double(1.0)/double(qtime)
      << setw(18) << setprecision(14) << tt_table[t]
...
/*----------------------------------*/
 ...
 i = 0;
 if ( datafile.is_open() ) {
   while ( datafile >> tmp1 >> tmp2 >> tmp3  ) ++i;
 } else {
...
*/
 corr = new double [nbin];

 datafile.open(filename.c_str());
 for ( i = 0; i < nline; ++i ) {
   datafile >> tmp1 >> tmp2 >> data[i][0];
 }
 datafile.close();

 datafile.open(filename.c_str());
 for ( i = 0; i < tot_pnt; ++i ) { 
   datafile >> tmp1 >> tt_table[i] >> tmp3 ;
 }
 datafile.close();
